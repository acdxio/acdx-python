...
class RestApi():
    ...
    def __init__(self, api_key='', key_secret='', uri='http://api.testnet.acdx.io'):
    ...
    def _get_authed_route_without_body(self, endpoint):
        ...
        self._headers['ACDX-ACCESS-KEY'] = self._api_key
        self._headers['ACDX-ACCESS-SIG'] = signature.decode().strip()
        self._headers['ACDX-ACCESS-TIMESTAMP'] = str(timestamp)
        return self.session.get(url=url, params="", headers=self._headers)
